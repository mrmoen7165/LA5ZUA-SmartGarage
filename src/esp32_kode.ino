...
/*************** WiFi og NTP ***************/
const char* WIFI_SSID      = "NavnetPåDittWiFi";
const char* WIFI_PASSWORD  = "PassordetTilDittWiFi";
...
const char* currentVersion = "1.1.7";   // Husk å oppdatere når du slipper ny firmware
...
void checkForUpdates() {
  ...
  Serial.println("\n[OTA] Sjekker etter oppdatering...");

  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("[OTA] Ikke tilkoblet WiFi. Hopper over OTA.");
    return;
  }
  ...
  if (httpCode == HTTP_CODE_OK) {
    ...
    Serial.printf("[OTA] Nyeste versjon på GitHub: %s\n", newVersion.c_str());
    if (newVersion != currentVersion) {
      Serial.println("[OTA] Ny versjon funnet! Starter oppdatering...");

      t_httpUpdate_return ret = httpUpdate.update(client, binURL);

      switch (ret) {
        case HTTP_UPDATE_FAILED:
          Serial.printf("[OTA] Oppdatering feilet: (%d): %s\n",
                        httpUpdate.getLastError(),
                        httpUpdate.getLastErrorString().c_str());
          break;
        case HTTP_UPDATE_NO_UPDATES:
          Serial.println("[OTA] Ingen ny oppdatering tilgjengelig.");
          break;
        case HTTP_UPDATE_OK:
          Serial.println("[OTA] Oppdatering fullført! Starter enheten på nytt...");
          break;
      }
    } else {
      Serial.println("[OTA] Ingen ny versjon tilgjengelig.");
    }
  } else {
    Serial.printf("[OTA] Feil ved henting av versjon.txt. HTTP-kode: %d\n", httpCode);
  }
...
<style>
@keyframes blinkUpdated {
  0%,100%{background:#0e2817;color:#aaffc3;}
  50%{background:#22c55e;color:#fff;}
}
.pill.updated{animation:blinkUpdated 1s ease-in-out 3;}
...
/* === Header med logo, tittel og OTA-knapp === */
.header {
  display:flex;
...
.logoText {
  ...
  text-shadow: 0 0 8px #22c55e, 0 0 12px rgba(34,197,94,.6);
  letter-spacing: 1px;
  transition: filter .2s, transform .1s, text-shadow .3s;
}
.logoText:hover {
  filter: brightness(1.2);
  text-shadow: 0 0 14px #22c55e, 0 0 24px #16a34a;
...
.otaBox .pill{
  ...
  background:#0e1a15;
  box-shadow:0 0 8px #0b2616 inset;
}
.otaBox .btn{
  padding:6px 12px;
  font-size:0.9rem;
...
.card{
  ...
  border-radius:14px;
  box-shadow:0 1px 0 #0a100e,0 0 24px #09120f inset;
  padding:14px;
  margin:12px 0;
}
...
.btn-chip{
  --bd:#34d399;--glow:#22c55e;color:#eafff2;background:linear-gradient(#113322,#0c2419);
  border:2px solid var(--bd);border-radius:999px;padding:10px 16px;font-weight:700;letter-spacing:.2px;
...
.btn-chip.active{background:linear-gradient(#0f3a22,#0d2f1d);border-color:#4ade80;box-shadow:0 0 0 2px rgba(255,255,255,.08) inset,0 0 18px var(--glow);}
...
/* 📱 Mobilvennlig justering for logo og OTA-knapp */
...
<!-- 🧭 Toppseksjon med LA5ZUA-tekst, tittel og OTA-knapp -->
<div class="header">
  <div class="titleWrap">
    <a href="https://github.com/mrmoen7165/LA5ZUA-SmartGarage" target="_blank" class="logoText">LA5ZUA</a>
    <h1>Balle’s Man Cave — Control Center</h1>
  </div>
  <div class="otaBox">
    <span class="pill" id="version">v1.1.4</span>
    <a id="otaBtn" href="javascript:runOTA()" class="btn">Oppdater OTA</a>
  </div>
</div>
...
<div class="wrap">
  ...
  <div class="layout">
    <div>
      ...
      <div class="card">
        <div class="grid">
          <div class="kv"><b>Temperatur måler DHT22</b><span id="temp" class="pill">-</span></div>
...
function runOTA(){
  const btn=E('otaBtn');
  btn.innerText='Oppdatering pågår...';
  btn.style.filter='brightness(0.6)';
  btn.style.pointerEvents='none';
  fetch('/action/ota')
  .then(res=>res.text())
  .then(()=>{
    btn.innerText='Oppdatering fullført ✅';
    E('version').classList.add('updated');
    setTimeout(()=>{
      E('version').classList.remove('updated');
      btn.innerText='Oppdater OTA';
      btn.style.filter='';
      btn.style.pointerEvents='';
    },15000);
  })
  .catch(()=>{
    btn.innerText='Feil ved OTA ❌';
    setTimeout(()=>{
      btn.innerText='Oppdater OTA';
      btn.style.filter='';
      btn.style.pointerEvents='';
...
void setup() {
  ...
  dht.begin();
  for (int i = 0; i < 10; i++) { delay(100); digitalRead(LM393_PIN); }
  ...
  // 🔄 Sjekk for OTA-oppdatering ved oppstart
  checkForUpdates();
...
const char* versionURL = "https://raw.githubusercontent.com/mrmoen7165/LA5ZUA-SmartGarage/main/firmware/version.txt";
const char* binURL     = "https://raw.githubusercontent.com/mrmoen7165/LA5ZUA-SmartGarage/main/firmware/latest.bin";
const char* currentVersion = "1.1.8";   // Husk å oppdatere når du slipper ny firmware
...
void checkForUpdates() {
  ...
  if (httpCode == HTTP_CODE_OK) {
    ...
    if (newVersion != currentVersion) {
      ...
      switch (ret) {
        ...
        case HTTP_UPDATE_OK:
          Serial.println("[OTA] Oppdatering fullført! Starter enheten på nytt...");
          delay(1000);
          ESP.restart();  // ✅ restart kun etter vellykket OTA
          break;
      }
    } else {
      Serial.println("[OTA] Ingen ny versjon tilgjengelig.");
    ...
    Serial.printf("[OTA] Feil ved henting av version.txt. HTTP-kode: %d\n", httpCode);
  }
...
/* === OTA-progressbar og animasjon === */
#otaProgressWrap {
  width:160px; height:6px; border:1px solid #2b3d34;
  border-radius:6px; background:#0b1411; overflow:hidden;
  box-shadow:0 0 8px #000 inset; margin-left:6px;
}
#otaProgressBar {
...
#version.updated {
  animation:blinkUpdated 1.5s ease-in-out 3;
}
...
<div class="header">
  ...
  <!-- ✅ Én enkelt statuslinje rett under progressbar -->
  <div id="otaStatus"
       style="font-size:0.8rem;opacity:.8;margin-left:6px;height:1.2em;"></div>
</div>
...
async function runOTA() {
  const btn = E('otaBtn');
  const bar = E('otaProgressBar');
  const ver = E('version');
  const statusTxt = E('otaStatus'); // 👈 ny linje for statusfeltet

  btn.innerText = 'Starter OTA...';
  btn.style.filter = 'brightness(0.6)';
  btn.style.pointerEvents = 'none';
  bar.style.width = '5%';
  bar.style.background = '#f59e0b'; // gul start
  statusTxt.innerText = 'Kobler til...'; // 👈 viser startstatus

  try {
    const response = await fetch('/action/ota');
    ...
    while (true) {
      ...
      // Finn sanntids fremdrift fra ESP
      const matches = [...text.matchAll(/progress:(\d+)/g)];
      if (matches.length) {
        progress = parseInt(matches[matches.length - 1][1]);
        bar.style.width = progress + '%';
        statusTxt.innerText = `Oppdaterer... ${progress}%`;

        if (progress < 50) bar.style.background = '#f59e0b'; // gul
        else if (progress < 90) bar.style.background = '#16a34a'; // lys grønn
        else bar.style.background = '#22c55e'; // mørk grønn
      }
    }

    // Ferdig OTA
    btn.innerText = 'Oppdatering fullført ✅';
    ver.classList.add('updated');
    bar.style.width = '100%';
    bar.style.background = '#22c55e';
    statusTxt.innerText = 'Oppdatering fullført ✅';

    setTimeout(refresh, 10000);

    setTimeout(() => {
      ver.classList.remove('updated');
      bar.style.width = '0%';
      btn.innerText = 'Oppdater OTA';
      btn.style.filter = '';
      btn.style.pointerEvents = '';
      statusTxt.innerText = ''; // rydder bort teksten
    }, 15000);
  } catch (err) {
    // Feilhåndtering
    ...
    bar.style.width = '100%';
    btn.innerText = 'Feil ved OTA ❌';
    statusTxt.innerText = 'Feil under OTA – sjekk WiFi eller GitHub-lenke.';
    setTimeout(() => {
      bar.style.width = '0%';
      btn.innerText = 'Oppdater OTA';
      btn.style.filter = '';
...
async function refresh(){
  try{
    ...
    S('soilraw',j.soil_raw);
    S('soilpct',j.soil_pct+' %');
    // Oppdater versjonsnummer automatisk fra ESP
    if (j.version) {
      E('version').innerText = 'v' + j.version;
    }
    setActive('btnGateAuto',j.gate_mode===0);
    setActive('btnGateManual',j.gate_mode===1);
    setActive('btnLightAuto',j.light_mode===0);
...
// ⚙️ OTA fra webgrensesnitt
server.on("/action/ota", []() {
  Serial.println("[WEB] OTA-oppdatering startet fra webgrensesnitt!");
  WiFiClientSecure client;
  client.setInsecure();
  HTTPClient http;
  ...
  String html = R"(
    ...
    <body style='font-family:system-ui;background:#0c1210;color:#e7efe9;'>
    <h2>OTA-oppdatering</h2>
    <pre id='log'>
  )";
  ...
  if (WiFi.status() != WL_CONNECTED) {
    sendLog("❌ Ikke tilkoblet WiFi. Hopper over OTA.");
    server.sendContent("</pre><p><a href='/'>Tilbake</a></p></body></html>");
    return;
  }

  sendLog("🔍 Sjekker etter ny firmware...");
  http.begin(client, versionURL);
  int httpCode = http.GET();
  ...
  if (httpCode == HTTP_CODE_OK) {
    ...
    sendLog("📦 Nyeste versjon på GitHub: " + newVersion);
    if (newVersion != currentVersion) {
      sendLog("⬇️ Laster ned ny firmware...");

      WiFiClientSecure client2;
      client2.setInsecure();

      // registrer OTA progress callback
      httpUpdate.onProgress([](int cur, int total){
        char buf[64];
        sprintf(buf, "progress:%d", (cur*100)/total);
        Serial.println(buf);
      });

      t_httpUpdate_return ret = httpUpdate.update(client2, binURL);

      switch (ret) {
        case HTTP_UPDATE_FAILED:
          sendLog("❌ Oppdatering feilet: " + httpUpdate.getLastErrorString());
          break;
        case HTTP_UPDATE_NO_UPDATES:
          sendLog("ℹ️ Ingen ny oppdatering tilgjengelig.");
          break;
        case HTTP_UPDATE_OK:
          sendLog("✅ Oppdatering fullført! Starter enheten på nytt...");
          delay(1000);
          ESP.restart();
          break;
      }
    } else {
      sendLog("✅ Ingen ny versjon tilgjengelig.");
    }
  } else {
    sendLog("❌ Feil ved henting av version.txt (" + String(httpCode) + ")");
  }
...
void loop() {
  ...
  // ⚙️ NY: OTA fra webgrensesnitt
  server.on("/action/ota", []() {
    server.send(200, "text/plain", "Starter OTA-oppdatering...");
    Serial.println("[WEB] OTA-oppdatering startet fra webgrensesnitt!");
    checkForUpdates();
  });
